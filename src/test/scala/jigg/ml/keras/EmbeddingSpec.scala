...
class EmbeddingSpec extends FlatSpec with Matchers{
  ...
  "convert" should "load model and convert input matrix" in {
    ...
    val inputData = csvread(new File(findPath("./data/ml/keras/embedding/embedding_input.csv")),separator = ',').map{x => x.toFloat}
    val goldData = csvread(new File(findPath("./data/ml/keras/embedding/embedding_gold.csv")),separator = ',').map{x => x.toFloat}
